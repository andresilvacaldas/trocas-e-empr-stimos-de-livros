...
class TransactionController {
    ...
    static async getMyTransactions(req, res) {
        try {
            ...
            if (status && Transaction.isValidStatus(status)) {
    ...
    static async getTransactionById(req, res) {
        try {
            ...
            const transaction = await Transaction.selectById({ 
    ...
    static async createTransaction(req, res) {
        try {
            ...
            const book = await Book.selectById({ book_id: parseInt(book_id) });
            if (!book) {
            ...
            const existingRequest = await Transaction.checkExistingRequest({
            ...
            if (transaction_type === 'troca' && offered_book_id) {
                const offeredBook = await Book.selectById({ 
            ...
            const transactionData = Transaction.prepareTransactionData(
    ...
    static async acceptTransaction(req, res) {
        try {
            ...
            const transaction = await Transaction.checkTransaction({ 
                transaction_id: parseInt(id) 
            });
    ...
    static async cancelTransaction(req, res) {
        try {
            ...
            const cancelled = await Transaction.delete({
                transaction_id: parseInt(id),
                requester_id: userId
            });
